...
class ResNet(nn.Module):
    ...
    def __init__(self, block, num_block, num_class=100, args=None):
        super().__init__()
        self.in_channels = 64
    ...
    def forward(self, x, label, train=False):
        ...
        if self.args.fcc > 0 and train:
            feature = self.fcc(feature.shape[0], feature, label, self.args.num_classes, self.args.fcc)
        if self.args.fcc_same > 0 and train:
            feature = self.fcc_same(feature.shape[0], feature, label, self.args.num_classes, self.args.fcc)
        if self.args.fcc_half > 0 and train:
            feature = self.fcc_half(feature.shape[0], feature, label, self.args.num_classes, self.args.fcc)
        if train is True:
            out = self.fc(feature)
            out_cb = self.fc_cb(feature)
            z = self.projection_head(feature)
